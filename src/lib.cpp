...
void menu(sf::RenderWindow& window) //функция вызова окна меню
{
    ...
    text1.setPosition(100, 30);//задаем позицию текста, отступая от центра камеры
    
    text2.setString("exit");//задаем строку тексту и вызываем сформированную выше строку методом .str() 
    text2.setPosition(100, 150);//задаем позицию текста, отступая от центра камеры

    text3.setString("rules: collect as many candies");//задаем строку тексту и вызываем сформированную выше строку методом .str() 
    text3.setPosition(300, 150);//задаем позицию текста, отступая от центра камеры
